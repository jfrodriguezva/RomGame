...
app/
  ...
  games/<slug>/           un material por carpeta (95)
components/
...
- `lib/speech.ts` usa la síntesis de voz del sistema en español, con tabla
  de fonemas.
- **`GameShell` puede decir la consigna en voz alta.** Prop opcional
  `hablarConsigna` (default `false`): cuando es `true`, un `useEffect`
  llama `hablar(consigna)` cada vez que el texto cambia. Activado en
  `MaterialOrdenar`, `MaterialTransferir` y `MaterialClasificar` — antes
  ninguno de los tres tenía ninguna voz (`MaterialOrdenar`/
  `MaterialTransferir`, 12 materiales, cero pistas sonoras; los 21 de
  `MaterialClasificar` solo hablaban la confirmación de acierto vía
  `textoVoz`, nunca la instrucción). `MaterialQuiz` se deja en `false`
  a propósito: ya tiene su propia voz sincronizada a las tres etapas, y
  hablar la consigna encima sonaría doble.
...
vez de un ámbar genérico. **Ya cubre los 91 materiales con niveles**:
...
**Ya cubre los 91 materiales con niveles**: 4 componentes compartidos +
...
- Tiempos y umbrales en `data/levels/` calibrados a ojo, no medidos con
  usuarios reales.
- Ninguno de los 96 materiales se ha probado interactivamente con un dedo
...
- **Punto 3 del roadmap (pulido visual/sonoro) cerrado.** `app/layout.tsx`
  ...
  animación de framer-motion, en los 96 materiales, respeta la
...
**Tercer bug real, mismo tipo, en la ronda de los 6 juegos de mesa:**
`domino` pedía más fichas por jugador de las que el mazo posible tenía en
niveles intermedios (nivel 7: 8 fichas repartidas, mazo de solo 6) —
`fichasPorJugador` y `imagenes` crecían con curvas independientes que se
desalinearon, no una curva copiada de otro material esta vez. Se arregló
derivando `fichasPorJugador` con un tope explícito (`Math.min(deseado,
Math.floor(totalFichasPosibles / 2))`) en vez de una tabla de paradas
suelta. `memoria-turnos`, `que-falta`, `oca` y `bingo` se revisaron con el
mismo método (simulación de los 100 niveles) y no tenían el problema.
Las 6 validaciones quedaron en `scripts/qa-levels.ts`.
