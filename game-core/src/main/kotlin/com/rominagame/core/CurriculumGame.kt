...
class CurriculumGame(familyId: String, private val onComplete: (String, Int, Int) -> Unit = { _, _, _ -> }) : ApplicationAdapter() {
    ...
    override fun create(){shapes=ShapeRenderer();batch=SpriteBatch();font=BitmapFont().apply{data.setScale(1.55f)};Gdx.input.inputProcessor=object:InputAdapter(){
        ...
        override fun touchUp(x:Int,y:Int,p:Int,b:Int):Boolean{val v=point(x,y);when{draggingSort->{draggingSort=false;dropSort(v.x,v.y)};draggingOrder>=0->{dropOrder(v.x);draggingOrder=-1};else->return false};return true}
    }}
    ...
    private fun tap(x:Float,y:Float){when(screen){
        Screen.MODES->family.modes.indices.forEach{i->val bx=60f+(i%3)*300f;val by=365f-(i/3)*92f;if(x in bx..bx+270f&&y in by..by+68f)start(i)}
        Screen.RESULT->if(y<175f){if(x<480f)screen=Screen.MODES else{level=(level+1).coerceAtMost(20);start()}}
        Screen.PLAY->{if(y>485f){when{x<145f->screen=Screen.MODES;x<300f->{level=(level-1).coerceAtLeast(1);start()};x<455f->{level=(level+1).coerceAtMost(20);start()}};return};when(mechanic()){
    ...
    private fun renderPlay(){button(18f,"MODOS");button(160f,"NIVEL -");button(305f,"NIVEL +");text("${family.modes[mode].title} · Nivel $level",700f,516f,Color.WHITE);text(challenge.prompt,480f,435f,Color(0xE0C23CFF.toInt()));when(mechanic()){CurriculumMechanic.QUIZ->renderQuiz();CurriculumMechanic.SORT->renderSort();CurriculumMechanic.ORDER->renderOrder();CurriculumMechanic.COUNT->renderCount()};text(feedback,480f,70f,Color.LIGHT_GRAY)}
