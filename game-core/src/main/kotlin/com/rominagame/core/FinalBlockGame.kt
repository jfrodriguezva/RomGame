...
class FinalBlockGame(familyId:String,private val onComplete:(String,Int,Int)->Unit={_,_,_->}):ApplicationAdapter(){
    ...
    override fun create(){shapes=ShapeRenderer();batch=SpriteBatch();font=BitmapFont().apply{data.setScale(1.6f)};Gdx.input.inputProcessor=object:InputAdapter(){
        ...
        override fun touchDragged(x:Int,y:Int,p:Int):Boolean{val v=point(x,y);if(mechanic()==FinalMechanic.DRAW&&screen==Screen.PLAY&&drawing){strokes+=Vector2(v.x,v.y);if(strokes.size>=45)finish();return true};return false}
        override fun touchUp(x:Int,y:Int,p:Int,b:Int):Boolean{drawing=false;if(!dragging)return false;val v=point(x,y);dragging=false;if(mechanic()==FinalMechanic.BASKET){if(v.x in 650f..830f&&v.y in 250f..370f)finish()else feedback="Lleva la pelota dentro de la canasta"}else if(v.x in 250f..710f&&v.y in 160f..390f){progress++;if(progress>=4)finish()else feedback="Pieza colocada"};return true}
    }}
    ...
    private fun start(selected:Int=mode){mode=selected;screen=Screen.PLAY;score=0;progress=0;strokes.clear();colored.clear();target=Vector2(300f+Random.nextFloat()*360f,190f+Random.nextFloat()*190f);patternGoal=List(4){(level+it)%4};pattern=List(4){(patternGoal[it]+1)%4};mazePos=0 to 0;feedback=instruction()}
    ...
    private fun tap(x:Float,y:Float){when(screen){Screen.MODES->family.modes.indices.forEach{i->val bx=90f+(i%3)*270f;val by=350f-(i/3)*115f;if(x in bx..bx+235f&&y in by..by+85f)start(i)};Screen.RESULT->if(y<175f){if(x<480f)screen=Screen.MODES else{level=(level+1).coerceAtMost(20);start()}};Screen.PLAY->{if(y>485f){when{x<145f->screen=Screen.MODES;x<300f->{level=(level-1).coerceAtLeast(1);start()};x<455f->{level=(level+1).coerceAtMost(20);start()}};return};when(mechanic()){
    ...
    private fun renderPlay(){button(18f,"MODOS");button(160f,"NIVEL -");button(305f,"NIVEL +");text("${family.modes[mode].title} · Nivel $level",700f,516f,Color.WHITE);when(mechanic()){FinalMechanic.DRAW->renderDraw();FinalMechanic.COLOR->renderColor();FinalMechanic.COLLAGE->renderCollage();FinalMechanic.MUSIC->renderMusic();FinalMechanic.PATTERN->renderPattern();FinalMechanic.TARGET->renderTarget();FinalMechanic.BASKET->renderBasket();FinalMechanic.MAZE->renderMaze()};text(feedback,480f,70f,Color.LIGHT_GRAY)}
