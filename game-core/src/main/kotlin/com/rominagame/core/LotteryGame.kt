...
class LotteryGame(private val bingo:Boolean,private val onComplete:(String,Int,Int)->Unit={_,_,_->}):ApplicationAdapter(){
 ...
 override fun create(){shapes=ShapeRenderer();batch=SpriteBatch();font=BitmapFont().apply{data.setScale(1.25f)};reset();Gdx.input.inputProcessor=object:InputAdapter(){
 ...
 private fun box(x:Float,y:Float,w:Float,h:Float,c:Color){shapes.begin(ShapeRenderer.ShapeType.Filled);shapes.color=c;shapes.rect(x,y,w,h);shapes.end()};private fun text(v:String,x:Float,y:Float,c:Color){wideText(v,x,y,c,110f)};private fun wideText(v:String,x:Float,y:Float,c:Color,w:Float=440f){batch.begin();font.color=c;font.draw(batch,v,x-w/2f,y,w,Align.center,false);batch.end()};private fun smallText(v:String,x:Float,y:Float,c:Color,w:Float){font.data.setScale(.72f);batch.begin();font.color=c;font.draw(batch,v,x-w/2f,y,w,Align.center,false);batch.end();font.data.setScale(1.25f)}
