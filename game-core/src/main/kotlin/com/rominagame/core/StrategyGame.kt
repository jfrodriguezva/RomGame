...
class StrategyGame(private val gameId:String,private val onComplete:(String,Int,Int)->Unit={_,_,_->}):ApplicationAdapter(){
 ...
 override fun create(){shapes=ShapeRenderer();batch=SpriteBatch();font=GameTypography.create(21);Gdx.input.inputProcessor=object:InputAdapter(){
 ...
 private fun box(x:Float,y:Float,w:Float,h:Float,c:Color){shapes.begin(ShapeRenderer.ShapeType.Filled);shapes.color=c;shapes.roundedRect(x,y,w,h);shapes.end()};private fun text(v:String,x:Float,y:Float,c:Color,w:Float=440f){batch.begin();font.color=c;font.draw(batch,v,x-w/2f,y,w,Align.center,false);batch.end()}
