...
class MainActivity:ComponentActivity(){
 override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);enableEdgeToEdge();val services=Services(applicationContext);setContent{MiAmbienteTheme{CompositionLocalProvider(LocalServices provides services){
  ...
  NavHost(navController=nav,startDestination=Ruta.INICIO){
   ...
   composable(Ruta.FAMILIA){entry->val context=LocalContext.current;FamiliaScreen(entry.arguments?.getString("id").orEmpty(),onAbrirJuego={id->if(id in setOf("gato","conecta4","rps","oca","serpientes","dado","solitario","arana-cartas","loteria","bingo","damas","damas-chinas","ajedrez","domino","adivinaquien"))context.startActivity(Intent(context,AlignmentGdxActivity::class.java).putExtra(AlignmentGdxActivity.EXTRA_GAME_ID,id))else nav.navigate(id)},onAbrirMotor={id->if(id=="memoria-observacion")context.startActivity(Intent(context,MemoriaGdxActivity::class.java))else context.startActivity(Intent(context,LogicBlockGdxActivity::class.java).putExtra(LogicBlockGdxActivity.EXTRA_FAMILY_ID,id))},onVolver=volver)}
